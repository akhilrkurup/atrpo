...
def collect_samples(pid, queue, env, policy, custom_reward,
    ...
    if pid > 0:
        ...
        if hasattr(env, 'np_random'):
            env.np_random = np.random.default_rng(env.np_random.integers(5000) * pid)

        if hasattr(env, 'env') and hasattr(env.env, 'np_random'):
           seed_value = env.env.np_random.integers(5000) * pid
           env.env.np_random = np.random.default_rng(seed_value)
    log = dict()
    memory = Memory()
    num_steps = 0
    total_reward = 0
