...
def update_critic(advantages, states):
    
    loss = 0.5 * (advantages** 2).mean()  # MSE on advantage error
...
def update_agent(rollouts, state_rms):  # Add state_rms as an argument
    ...
    norm_next_states = torch.tensor(state_rms.normalize(next_states.numpy()), dtype=torch.float32)
    ...
    for i, r in enumerate(rollouts):
        ...
        norm_r_next_states=torch.tensor(state_rms.normalize(r.next_states.numpy()), dtype=torch.float32)
        #rewards = torch.cat([r.rewards for r in rollouts], dim=0)
        advantages.append(estimate_advantages(norm_r_states, norm_r_next_states, r.rewards))  # Use normalized states
...
def estimate_advantages(states, next_states, rewards, gamma=0.99):
    with torch.no_grad():
        next_values = critic(next_states)  # No gradient needed for next state values
        #print("Critic values:", critic(states).mean().item(), critic(states).std().item())

        td_target = rewards + gamma * next_values

    values = critic(states)  # This must require gradients

    advantages = td_target - values  # values has grad, td_target does not (safe)
    return advantages
